...
his exercise builds upon the early [model creation](./13_model_building.html), using the outputs generated during that
exercise.
...
is input to the method call. A simple way
...
ctch_mdl$plot_state("s_sz") ## TOD add map ealier
```

## Changing Parameters

The parameters are stored within the HRU descriptions; particuarly the `sf`, `rz`, `uz` and `sz` elements. Each of these contains a `type` variable and corresponding names vector of parameters.
...
In the follwoing presents a simple function of setting a faster surface velocity and kinematic solution for the river channels.
