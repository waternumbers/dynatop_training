...
going to be partially captures by spatial data. Past TOPMODEL and Dynamic
...
```{r, echo=FALSE, out.width="75%", fig.align="center", fig.cap="Map of topographic index classes"}
...
```{r, echo=FALSE, out.width="75%", fig.align="center", fig.cap="Map of land use classes"}
...
assumed at it receives all the outward flow. The following figure shows the
connectivity for the small catchment.

```{r, echo=FALSE, out.width="75%", fig.align="center", fig.cap="Map of combined topographic and land use classes along with connectivity"}
knitr::include_graphics("./images/combined_class.png")
```

### Classification (Part 2)

Each class is represented by a single HRU, in inflows to which are
...
The cells in the catchment can be banded using the connectivity - all cells
in a given band receive flows only from those in a higher band. For the small
catchment this looks like the following

```{r, echo=FALSE, out.width="75%", fig.align="center", fig.cap="Map of the band classification"}
knitr::include_graphics("./images/band_class.png")
```

Combing this into the classification gives

```{r, echo=FALSE, out.width="75%", fig.align="center", fig.cap="Map of the
...
from the river. This gives a further level of spatial refinement to the
...
> other options could be used it is **strongly** recommended that the band is
> included in all classifications.

## Hydrological Response Units

Each type of Hydrological Response Unit HRU represents a parameterised version
of a different component of the hydrological system. Two types of HRU are
available in `dynatop`, which represent river channel lengths and hill slopes.

### Channel HRU

The Channel HRU is designed to represent a length of river channel. It
implements a time delay histogram routing of the inflow to the river reach to
a specified point on the river channel network (e.g. a gauge site) using a time
delay histogram, based on constant channel velocity. 

Details of the channel HRU are given in a `dynatop`
[vignette](https://waternumbers.github.io/dynatop/articles/Channel_HRU.html).
part of the catchment

### Hill Slope HRU

The Hill slope HRU computes the outflow for a representative hill slope whose
shape is determined by the total surface area and width across which the
outflow occurs. 

The hill slope consists of four zones:
...
```{r, echo=FALSE, out.width="75%", fig.align="center", fig.cap="Schematic of
the Hill slope HRU"}
...
the surface and the saturated zones. Details if the governing equation and
numerical solution are given [elsewhere
](https://waternumbers.github.io/dynatop/articles/Hillslope_HRU.html). A few
key points are:

- The lateral flow from the Surface Zone is proportional to the Surface Zone storage
...
storage deficit through a transmissivity profile which gives the lateral flow per
unit outflow width.

The second part of the training introduces more details of the numeric solution.
