...
```{r, read_raster}
dem <- rast(file.path(".","eden_data","unprocessed","Eden_DEM.tif")) # load the dem as a raster layer
eden <- vect(file.path(".","eden_data","unprocessed","Eden_Catchment.shp")) # load the outline of the sub-catchments from the shapefile
channel <- vect(file.path(".","eden_data","unprocessed","Eden_River_Network.shp")) # load the river network from the shapefile
urban <- vect(file.path(".","eden_data","unprocessed","Eden_Urban.shp")) # load the urban area map from the shapefile
```
...
csvGauges <- read.csv(file.path(".","eden_data","unprocessed","Eden_Gauge_Sites.csv"))
...
```{r, save_eden_raster}
writeRaster(eden_raster,file.path(".","eden_data","processed","eden.tif"),overwrite=TRUE)
writeRaster(urban_raster,file.path(".","eden_data","processed","urban.tif"),overwrite=TRUE)
writeRaster(dem,file.path(".","eden_data","processed","dem.tif"),overwrite=TRUE)
```
...
```{r, save_eden_channel}
writeVector(conv_channel,file.path(".","eden_data","processed","channel.shp"),overwrite=TRUE)
```
...
writeVector(gauges,file.path(".","eden_data","processed","gauges.shp"),overwrite=TRUE)
```
